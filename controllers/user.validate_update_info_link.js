module.exports = (req, res, next)=>{
    ...
    if ( password ) 
        if ( !regExPassword.test(password)) 
            return res.json({ message: 'password illegal', success: true })
    if ( customLink )
        if ( !regExCustomLink.test(customLink))
            return res.json({ message: 'custom link illegal', success: true })
    if ( expire )
        if ( !regExExpire.test(expire))
            return res.json({ message: 'expire illegal, only use 1-100', success: true })
    return next()
}